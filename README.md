...
首页提供作品封面、收藏筛选、随机观看和 HTML 自助导入。选择或拖入一个或多个 `.html` / `.htm` 文件后，页面会自动读取网页标题和文件最后修改时间，以内容指纹去重，并将作品存入 IndexedDB。刷新页面后仍会保留；作品支持实时预览、观看、暂停、重播、下载和删除。
...
2026-09-17：新增浏览器端 HTML 自助导入。通过真实 Chromium 验证文件选择、标题识别、内容去重、IndexedDB 持久化、筛选、沙盒观看、暂停、重播、下载和删除，并检查桌面与 390px 手机布局。
